...
* **Class SparseMatrix** : Define uma estrutura de dados que é uma matriz esparsa. Possui uma nested class `DataNode` e implementa duas interfaces, a `IMatrix` e a `Cloneable`. 
  ...
  *  *setElement*, que dado um `int` i, `int` j e um `T` value, coloca num `DataNode<T>` , com a respetiva possição ese value , sendo que se ele não  existir, cria-o primeiro. 
    *  *getElement* que dado um `int` i e um `int` j retorna o value `T` existente no `DataNode<T>`, se este existir. 
