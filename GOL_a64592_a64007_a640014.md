...
​	A classe `SparseMatrix` implementa ainda  uma interface criada por nós, denominada `IMatrix`,  assim como a interface `Cloneable`. A inner class class `SentinelIterator` implementa a interface `Iterator`. A classe `SMBoard` implementa também uma interface criada por nós, denominada`IBoardGoL` . 
...
* **Class SentinelNode**: Define um SentinelNode. Esta classe é estática, para permitir que esta possa ser instanciada, sem que seja necessário a instanciação da sua outer class. 
  ...
  *  Possui um construtor com apenas um parâmetro, um `int` number. Permite a sua representação em formato `String` através da função *ToString*.  Uma vez que esta classe é apenas utilizada na construção da estrutura de dados `SentinelLL`, encontra-se lá aninhada.
...
* **Class SparseMatrix**: Define uma estrutura de dados que é uma matriz esparsa. Possui uma nested class `DataNode` e implementa duas interfaces, a `IMatrix` e a `Cloneable`. 
  ...
  * Possui dois construtores, um que aceita um `ArrayList<String> ` input, e outro que aceita quatro parâmetros, um `int` *startingRow*, um `int` *startingColumn*, um `int` *endRows* e um `int` *endColumns*. De acordo com o contrato da interface `IMatrix`,  possuindo as seguintes funções :
...
* Esta classe implementa a interface `Iterator`, uma vez que é um iterador, pelo o que é obrigatório a implementação de três funções : 
  ...
  *  a *next* que retorna o `SentinelNode` iterado e atualiza o *nextNumber*. 
  *  *remove*, sendo que esta última foi apenas declarada, não existindo corpo da função pois neste caso não faz sentido remover elementos da `SentinelLL` enquanto se itera.
